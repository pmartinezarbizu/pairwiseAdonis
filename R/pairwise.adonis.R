...
pairwise.adonis <- function(x,factors, sim.function = 'vegdist', sim.method = 'bray', p.adjust.m ='bonferroni',reduce=NULL,perm=999, binary=FALSE)
{
  ...
  R2 <- c()
  p.value <- c()
  tot_num_comps <- length(co)/2
  print(paste('total number of comparisons = ', length(co)/2))
  
  pb <- txtProgressBar(style = 3)
  
  current_comp <- 1
  for(elem in 1:ncol(co)){
    percent_done <- current_comp/tot_num_comps
    setTxtProgressBar(pb, percent_done)
    ...
    if(inherits(x, 'dist')){
      ...
      if (sim.function == 'daisy'){
        ...
        x1 = vegdist(x[factors %in% c(co[1,elem],co[2,elem]),],method=sim.method, binary=binary)
      }
    )
    ...
    #print(paste('calculating comparison', current_comp))
